...
const PopularProductCard = ({ id, imgURL, name, price }) => {
  return (
    <Link to={`/products/${id}`} className="block">
      <div className="flex flex-1 flex-col w-full max-sm:w-full " role="article" aria-label={name}>
        <img
          src={imgURL}
          alt={name}
          className="w-[290px] h-[280px] object-cover rounded-lg mb-4"
        />
        ...
        <p className="text-gray-600 font-montserrat">{price}</p>
      </div>
    </Link>
  );
};
